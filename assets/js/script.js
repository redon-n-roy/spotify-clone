...
function addPlaylistArt(playlistId) {
	...
	if(jQuery.inArray(imageExtension, ['gif', 'png', 'jpg', 'jpeg']) == -1) {
		alert("Invalid image file. Check file extension.");
	}
	var imageSize = property.size;
	if(imageSize > 2000000) {
		alert("Image file size is large. Please upload smaller image.")
	}
	else {	
		var requiredData = new FormData();
		requiredData.append("file", property);
		requiredData.append("playlistId", playlistId);
		$.ajax({
			url: "includes/handlers/ajax/addPlaylistArt.php",
			method: "POST",
			data: requiredData,
			contentType: false,
			cache: false,
